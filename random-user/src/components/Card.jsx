...
function Card() {
   ...
   const fetchRandomUser = useCallback(async()=>{
      try {
         const dataRes = await axios.get(import.meta.env.VITE_RANDOM_USER_API);
         console.log(dataRes.data);
         setResponse(dataRes.data.data);
      } catch (error) {
         console.log("Error Api Related: ", error.message);
      }
      finally{
         setLoading(false)
            ...
            <Container>
               ...
               <div className="h-52 w-full pt-4 ">
                  ...
                  <div className="text-center py-4 space-y-3">
                     ...
                     <p className="text-sm">{response?.login?.username}</p>
                  </div>
